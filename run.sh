...
if [ ! -n "$WERCKER_GIT_MERGE_NAME" ]; then
  WERCKER_GIT_MERGE_NAME = $(git config --get user.name);
fi

if [ ! -n "$WERCKER_GIT_MERGE_EMAIL" ]; then
  WERCKER_GIT_MERGE_EMAIL = $(git config --get user.email);
fi

regex="^[a-z0-9!#\$%&'*+/=?^_\`{|}~-]+(\.[a-z0-9!#$%&'*+/=?^_\`{|}~-]+)*@([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)+[a-z0-9]([a-z0-9-]*[a-z0-9])?\$"

if [[ ! $WERCKER_GIT_MERGE_EMAIL =~ $regex ]] ; then
...
git config user.email "$WERCKER_GIT_MERGE_EMAIL"
git config user.name "$WERCKER_GIT_MERGE_NAME"
